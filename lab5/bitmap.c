...
struct pixel **read_pixel_array(FILE *image, int pixel_array_offset, int width, int height) {
  struct pixel **px_array = malloc(sizeof(struct pixel *) * height);
  fseek(image, pixel_array_offset, SEEK_SET);
  for (int i = 0; i < height; i++){
    // make each element representing a row
    px_array[i] = malloc(sizeof(struct pixel) * width);
    fread(px_array[i], 3, width, image);
  }
  return px_array;
